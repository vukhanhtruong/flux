...
class S3StorageProvider:
    ...
    async def upload(self, file_path: Path, key: str) -> str:
        ...
        await loop.run_in_executor(
            None, self._client.upload_file, str(file_path), self._bucket, s3_key
        )
        return s3_key
    ...
    async def download(self, key: str, dest: Path) -> Path:
        ...
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(
            None, self._client.download_file, self._bucket, key, str(target)
        )
        return target
