...
def test_get_db_creates_database_when_none(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(infra, "_db", None)
    monkeypatch.setattr(infra, "Database", mock_db_class)
    monkeypatch.setattr(infra, "migrate", mock_migrate)
...
def test_get_embedding_service_creates_service_when_none(monkeypatch):
    ...
    monkeypatch.setattr(infra, "_embedding_service", None)
    monkeypatch.setattr(infra, "EmbeddingService", mock_svc_class)
